...
it("renders owner-only Qonto and Tiime state", async () => {render(await Page()); expect(screen.getByRole("heading", {name:"Intégrations"})).toBeInTheDocument(); expect(mocks.getQontoIntegration).toHaveBeenCalledWith({},"owner"); expect(screen.getByText("Demande API en attente")).toBeInTheDocument();});
