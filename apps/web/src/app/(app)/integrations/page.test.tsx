...
it("renders owner-only Qonto and Tiime state", async () => {render(await Page()); expect(screen.getByRole("heading", {name:"Intégrations"})).toBeInTheDocument(); expect(mocks.getQontoIntegration).toHaveBeenCalledWith({},"owner"); expect(screen.getByRole("link", {name:"Configurer Tiime"})).toHaveAttribute("href", "/integrations/setup#tiime"); expect(screen.queryByText(/Offre Essential|Grow ou supérieur|Clé API personnelle/)).not.toBeInTheDocument();});
