...
it("labels a Qonto opening balance and its stale publication without changing reserve semantics", () => {
 ...
 expect(screen.getByRole("button", {name:/Solde Qonto/})).toBeInTheDocument(); expect(screen.queryByText(/2026-09-10T/)).not.toBeInTheDocument(); expect(screen.getByText(/actualisation nécessaire/i)).toBeInTheDocument(); expect(screen.getByText(/USD/)).toBeInTheDocument(); expect(screen.queryByText("solde manuel actuel")).not.toBeInTheDocument();
});


it("formats a manual business date without shifting it with the owner timezone", () => {
  render(<KpiStrip kpis={{currentBalanceCents:moneyCents(500000),availableBalanceCents:moneyCents(400000),inflows30DaysCents:moneyCents(0),outflows30DaysCents:moneyCents(0),projected30DaysCents:moneyCents(500000),runwayDays:null}} horizonDays={90} scenario="certain" openingBalanceSource="manual" openingBalanceAsOf="2026-09-10" timezone="America/Los_Angeles" excludedBankCurrencies={["USD"]} />);
  expect(screen.getByText("Solde manuel au 10/09/2026")).toBeInTheDocument();
